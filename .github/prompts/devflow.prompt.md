...
Read `AGENTS.md`, load `skills/devflow-core/SKILL.md`, and select Problem, Fast, Design-lite, Design, Build, or Recovery from current facts. Apply Core's risk gate: clear local reversible existing-feature changes may enter Cut directly; ambiguous or materially risky creative work selects `devflow-brainstorm` for a `Confirmed request` and user-selected A/B/C. Pure Q&A, lookup, verification, investigation-only reports, and already approved changes are exceptions.
