...
3. Spec, Cut, or Plan: `devflow-spec` saves a requirements spec under `docs/specs/`. `devflow-cut` decides reuse, scope, and constraints before planning. After `CUT_PASS`, `/devflow-plan` creates a construction Plan Pack for Depth A/B; Depth C goes directly from Cut to Build. Spec and plan have a STOP gate for user review; plan review then performs only lightweight Cut-consistency review.
