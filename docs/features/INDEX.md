...
| DevFlow 计划生成与落地 | 把 CUT_PASS 变成按交付单元拆分、跨会话/跨模型可执行的施工单；Build 回写进度、Prove 回写 Status 与 Landed | 计划, plan, 任务拆解, 任务粒度, 交付单元, 拆分, 跨会话, 跨模型, right-sizing, 施工单, Progress, 落地, Landed, 计划校验 | `/devflow-plan` | active | `node scripts/devflow-plan.js plan-file` | skills/devflow-plan/SKILL.md, scripts/devflow-plan.js | 2026-09-10-devflow-usability-flow-reconfiguration.md | 2026-09-11 |
