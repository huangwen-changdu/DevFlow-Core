...
- Current Version: v55
- Status: active
- Last Change: find-fault-adjustment-list
...
- Direct success ownership is explicit: A starts Spec then continues Cut -> Plan -> Build -> Prove; B continues Cut -> Plan -> Build -> Prove; C continues Cut -> Build -> Prove. Only the named success state has this bypass, never an exception state.
- `devflow-plan` is the single plan-generation skill behind the unchanged `/devflow-plan` command: A/B `CUT_PASS` starts it directly; it writes reviewed construction Plan Packs with `Source` tracing, per-task file anchors, a `## Recon` record pairing commands with their output, task acceptance and verification, and an approval stop. Plan review performs only a lightweight Cut-consistency check, then an approved A/B Plan directly enters Build while scope-drift facts return to Core. It does not prescribe test-first development, version-control task steps, independent review, or plan execution.
...
- Saved specs default to `docs/specs/YYYY-MM-DD-<short-kebab-name>.md`; saved plans default to `docs/plans/YYYY-MM-DD-<short-kebab-name>.md` and must cite `Source:` plus a `## Recon` record of the bounded read.
...
| v48 | core-exclusive-lifecycle-routing | lifecycle boundary | 2026-07-28 | superseded by v50 | Extended Core's unique lifecycle routing from `Confirmed request` and `Confirmed Spec` to Cut Decisions, confirmed Plans, and PUA recovery facts. v50 narrows that ownership to non-unique states. |
...
| v43 | adversarial-review-confirmation-pause | manual review safety | 2026-07-27 | superseded by v51 | Added a confirmation pause inside `devflow-adversarial`: every run warns that the review may take a long time, and does not inspect materials or begin the five-angle review until the user explicitly confirms. |
...
- 2026-07-30: Use a three-layer context contract instead of compressing the same lifecycle text in every host rule. Reason: startup interfaces need only route, owner location, host fallback/load action, and proof exit; Core can load shared methods first and the selected lifecycle owner can load its local detail without losing compatibility for hosts that do not auto-load skills.
...
- Host adapters are intentionally incomplete lifecycle narratives. They must preserve a route signal, Core or direct-review owner location, host fallback/load action, and proof exit; detailed execution stays with its single runtime owner.
- Every runtime reference named by Core or an installed skill must be copied by both applicable installers. Source-package documentation remains outside installed target runtimes unless it is required at execution time.
