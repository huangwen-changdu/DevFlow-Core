// @devflow-core/dsh-devflow — asset sync helper (maintainer tool).
// Copies the DevFlow assets from the repository root into this package's
// self-contained assets/ tree so the npm tarball carries the current
// preset/skills/commands/scripts. MUST be re-run before every publish;
// otherwise the packaged assets drift from the repo sources.
//
...
// Repository root = ../../../.. relative to this file (dsh/plugins/dsh-devflow/scripts/).
...
const groups = [
  { src: 'dsh/agent-presets', dst: 'presets', pick: (name) => name === 'devflow-2' },
  { src: 'skills', dst: 'skills', pick: (name) => name.startsWith('devflow-') },
  { src: 'commands', dst: 'commands', pick: (name) => name.startsWith('devflow') },
  { src: 'scripts', dst: 'scripts', pick: (name) => checkerScripts.includes(name) },
]
...
console.log(`DevFlow DSH plugin assets synced into ${relative(root, outDir)}`)
