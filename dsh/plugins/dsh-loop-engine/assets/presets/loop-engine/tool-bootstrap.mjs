/**
 * loop-engine adaptation: identical two-phase bootstrap as devflow-2 /
 * dsh-liangshen, with the persona swap configured through `phase1Persona`.
 * Phase 1 shows the one-line Minimal anchor; after promotion the assembled
 * persona section (the preset's full Loop Engineer prompt from
 * agent.cordis.yml) is restored untouched, and the workspace line is only
 * appended when the persona does not already mention a working directory
 * (the Loop Engineer template resolves {{cwd}} itself).
 *
 * Synced with dsh-liangshen 0.3.14 (via devflow-2): goal whitelist (#578),
 * presentation broadcasts (#1128), and the snapshotEvents() session-events
 * fallback for newer DSH releases. With promotedPresentation: native the
 * Code Mode switch and its broadcast are no-ops; everything else is shared.
 */
 ...
 * string — including `code`, which `dsh-tools` falls through to the default
 * branch — = every tool schema AND `run_code` AND the SDK section, i.e. one
 * catalog paid for twice. Only `ptc` narrows `wireSchemas()` to the
 * `run_code` transport. The switch happens at the step boundary so the
 * current step's native calls are never interrupted. `deferredSources` and
