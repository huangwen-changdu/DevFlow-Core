...
| Design | New requirement, behavior change, feature, architecture change, ambiguity, multi-solution decision, or unclear small-feature boundary. Not for documentation writing, config tuning, or clear small changes to existing features. | Sense -> Brainstorm clarification -> Core consumes `Confirmed request` -> Core selects Spec when a design contract/saved requirements source is needed -> confirmed Spec returns to Core -> Core selects Cut, Plan, Build, Prove, or no further work. Skip Brainstorm for doc/config/clear-fix tasks — use Design-lite or Fast. |
| Build | User asks to implement, fix, land, or execute a change. | Sense -> Brainstorm clarification when needed -> Core consumes `Confirmed request` and selects the smallest safe lifecycle path -> Cut -> Build -> Prove. Skip Brainstorm/Plan if already completed, if root cause is clear and fix is trivial, or for doc/config work — use Design-lite or Fast. |
| Recovery | The user repeatedly reports that the same function, result, or requested capability remains wrong, incomplete, or missing in one task lifecycle, including after an attempted correction; unexpected verification failure; or giving-up impulse. | Load `devflow-pua` when pressure recovery is needed; diagnose user-view miss -> re-read facts -> 3 hypotheses -> different/opposite method -> changed approach -> Prove |
...
| "requirement", "feature request", "add support", "implement" | Requirement | `devflow-core -> devflow-brainstorm -> Confirmed request -> devflow-core`; Core chooses Spec when the request needs approach comparison or a saved design contract. Spec compares options, writes the contract, waits for user approval, then returns the confirmed Spec to Core. Core selects Cut, Plan, Build, Prove, or no further work. For documentation, config, or clear small fixes, use Design-lite or Fast instead of Brainstorm. |
| "bug report", "error", "failing test", "fix bug", "broken" | Bug fix | Root-Cause Check first. If root cause is clear and fix is trivial → Fast or Design-lite. Otherwise `devflow-core -> devflow-brainstorm -> Confirmed request -> devflow-core -> devflow-cut -> Cut Decision -> devflow-core -> devflow-build -> devflow-prove`. |
| repeated report that the same function, result, or requested capability is wrong, incomplete, or missing | Pressure recovery | `devflow-core -> devflow-pua -> recovery facts -> devflow-core`; Core may select `devflow-brainstorm` only to re-confirm the request, then chooses later lifecycle work. |
...
|---|---|---|
| unclear ask, new requirement, behavior change, multiple paths, or non-trivial clarification | Brainstorm Clarification | Load `devflow-brainstorm` to read facts, send Semantic Echo-Back, resolve only real request gaps one at a time, and produce `Confirmed request`. After `Status: clarified`, `devflow-core` consumes the summary. Core decides whether the request needs `devflow-spec`; when selected, Spec compares real options, writes a design contract/saved spec, waits for approval, and returns the confirmed Spec to Core. Only Core then selects `devflow-cut`, `devflow-plan`, `devflow-build`, `devflow-prove`, or no further lifecycle work. Brainstorm and Spec never choose a path, depth, or downstream skill. |
...
| task matches an available non-devflow skill (e.g., frontend-design, pdf, docx, understand, data-analysis) | External Skill Dispatch | Suggest loading the matched skill alongside the devflow route. External skills and devflow are complementary: devflow manages scope, risk, and minimum change (what to change); external skills guide execution quality (how to do it well). After any needed Brainstorm clarification, Core owns the lifecycle route. `CUT_REUSE` applies only when the external skill fully handles the task with no new code needed (e.g., `pdf` for reading a PDF, `understand` for codebase analysis). Record `External Skill: <name> (matched: <why>)`. The match travels down the chain: Cut records it as `External Skills` in the Cut Decision, the Plan Pack inherits it in its header and merges the skill's quality checks into task `Acceptance`/`Verify`, and Build loads it before editing or records the exception. |
...
| implementation ready | Build Discipline | Load `devflow-build`; require Plan Review, touched files, slices, diff self-check. |
| build hits a blocker, stale plan, unclear instruction, or repeated verification failure | Build Stop Protocol | Receive `BUILD_BLOCKED` facts from `devflow-build`; decide whether to revise the plan, re-clarify with `devflow-brainstorm`, or enter `devflow-pua`. Never tell Build to guess past a blocker. |
...
| repeated report that the same function, result, or requested capability remains wrong, incomplete, or missing | Pressure Recovery Gate | Load `devflow-pua`; stop editing, quarantine previous wrong assumptions, list 3 hypotheses, switch method, return recovery facts to Core, then let Core select any needed Brainstorm, Cut, Plan, Build, Prove, or no further work. |
...
```text
Goal: ...
Needs Brainstorm clarification: yes/no
Confirmed request: present/not needed
Needs Spec design contract: yes/no
Confirmed Spec: present/not needed
Needs Cut: yes/no
Implementation requested: yes/no
Next skill: selected by devflow-core after confirmed request, confirmed Spec, Cut Decision, confirmed Plan, or recovery facts
```
...
Repeated target evidence: <same target + prior feedback or correction attempt>
...
| "This is a new feature but it's small, so Design-lite." | New features go through Brainstorm. Design-lite is for existing features only; after `Confirmed request`, Core selects the smallest safe lifecycle path. |
...
- Design route -> `devflow-brainstorm` for clarification; after `Confirmed request`, `devflow-core` decides whether to invoke `devflow-spec`; after a confirmed Spec, `devflow-core` selects the next lifecycle skill
- Before new structure -> `devflow-cut`; all Cut results return to Core, and after `CUT_PASS` Core decides whether planning, Build, Prove, or no further lifecycle work is needed
- Approved construction plan -> return the confirmed Plan and any scope-drift facts to Core; Core selects any later lifecycle work
- Task matches available external skill -> suggest loading that skill alongside the devflow route; devflow manages scope/risk, external skill guides execution quality; devflow chain always runs; the match is recorded as `External Skills` in the Cut Decision, inherited by the Plan Pack header, and loaded by Build
- Build blocked -> `devflow-build` returns `BUILD_BLOCKED` facts to `devflow-core`; Core selects plan revision, `devflow-brainstorm` re-clarification, or `devflow-pua`
...
- User challenge, explicit wrong-code signal, changed-wrong result, repeated miss, repeated "少了这个/少个那个" feedback, missing-piece complaint, or quality complaint -> `devflow-pua -> recovery facts -> devflow-core`; Core may select `devflow-brainstorm` only for request re-confirmation
