...
// 覆盖：subtract（曾为故意 bug，本循环修复）、multiply、divide 两态（正常除 + 除零抛错）、divide2 两态。
const assert = require('assert');
const { subtract, multiply, divide, divide2 } = require('./calc.js');
...
assert.strictEqual(divide2(6, 3), 2); // divide2 正常：6 / 3 === 2
assert.throws(() => divide2(1, 0), /zero/); // divide2 除零抛出错误
