...
STOP gates are mandatory wait points — do not continue until the user responds: brainstorm (echo-back confirmation, path selection Fast Exit vs A/B/C, depth selection A/B/C, design contract [A/B only]), spec (review written spec), /devflow-plan (review plan), cut (CUT_REDUCE/CUT_REUSE confirmation).
## Skills
...
- `devflow-spec`: turns an approved design contract into `docs/specs/YYYY-MM-DD-<short-kebab-name>.md` for traceability. Hands off to `devflow-cut`; Depth A enters `/devflow-plan` only after `CUT_PASS`.
- `/devflow-plan` (command): creates an implementation construction Plan Pack from `CUT_PASS` plus a spec or approved design. After plan review and lightweight Cut-consistency review, hands off to `devflow-build`.
...
Required Gates block construction planning and Build: Reuse, Ponytail Rung, Root-Cause (bug fixes), Native, Overbuild, Diff, Scope. Cut results: `CUT_PASS` -> Depth A/B plan -> build, Depth C -> build; `CUT_REDUCE`/`CUT_REUSE` -> STOP for user confirmation; `CUT_BLOCKED` -> return to `devflow-brainstorm`.
