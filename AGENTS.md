...
STOP gates are mandatory wait points — do not continue until the user responds: brainstorm (STOPs: depth selection A/B/C, core clarification, design contract; then depth-based: A=spec+plan, B=plan, C=direct to cut), spec (review written spec), /devflow-plan (review plan), cut (CUT_REDUCE/CUT_REUSE confirmation).
